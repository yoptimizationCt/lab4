...
def get_gradient(point, j):
    return np.array(
        [2 * X[j] * (point[0] * X[j] + point[1] - Y[j]),
         2 * (point[0] * X[j] + point[1] - Y[j])])


def gradient(point, i, batch_size):
    gr = np.zeros(len(point))
    number_of_coords = len(X)
    for j in range(batch_size):
        gr += get_gradient(point, (i * batch_size + j) % number_of_coords)
    return gr


def SGD(lr=0.0009, epochs=100, batch_size=1):
    points = np.zeros((epochs + 1, 2))
    num_epoch = epochs
    for epoch in range(1, epochs + 1):
        # j = np.random.randint(0, n)
        gr = gradient(points[epoch - 1], epoch, batch_size)
...
def SGD_with_momentum(lr=0.0009, v=np.zeros(2), gamma=0.5, epochs=100, batch_size=1):
    points = np.zeros((epochs + 1, 2))
    num_epoch = epochs
    for epoch in range(1, epochs + 1):
        j = np.random.randint(0, n)
        gr = gradient(points[epoch - 1], j, batch_size)
        if (abs(gr) < eps_point).all():
            num_epoch = epoch
            break
...
def Nesterov(lr=0.0009, v=np.zeros(2), gamma=0.5, epochs=100, batch_size=1):
    points = np.zeros((epochs + 1, 2))
    num_epoch = epochs
    for epoch in range(1, epochs + 1):
        j = np.random.randint(0, n)
        gr = gradient(points[epoch - 1] - lr * gamma * v, j, batch_size)
        if (abs(gr) < eps_point).all():
            num_epoch = epoch
            break
        v = gamma * v + (1 - gamma) * gr
...
def AdaGrad(lr=1, epochs=100, eps=10e-8, batch_size=1):
    state_sum = 0
    points = np.zeros((epochs + 1, 2))
    num_epoch = epochs
    for epoch in range(1, epochs + 1):
...
def RMS_prop(lr=0.1, epochs=100, alpha=0.5, eps=10e-8, batch_size=1):
    points = np.zeros((epochs + 1, 2))
    s = 0
    num_epoch = epochs
    for epoch in range(1, epochs + 1):
...
def Adam(lr=0.1, epochs=100, eps=10e-8, beta1=0.9, beta2=0.999, batch_size=1):
    points = np.zeros((epochs + 1, 2))
    s = 0
    v = 0
    num_epoch = epochs
...
def draw_line(points, file):
    plt.clf()
    plt.rcParams["figure.figsize"] = (10, 10)
    fig, ax = plt.subplots()
    ax.scatter(X, Y)
    ax.plot([0, 10], [points[-1][1],
                      10 * points[-1][0] + points[-1][1]], color='red', linewidth=5)
    plt.savefig(file)
